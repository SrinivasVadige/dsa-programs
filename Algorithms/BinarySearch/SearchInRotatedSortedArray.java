...
public class SearchInRotatedSortedArray {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("binarySearch using one binary search => " + searchUsingOneBinarySearch(nums, target));
    }
     ...
     * @SpaceComplexity O(1)
     */
    public static int searchUsingOneBinarySearch(int[] nums, int target) {
        int l = 0, r = nums.length - 1;
        while (l <= r) {
            int mid = (l + r) / 2;
            if (nums[mid] == target) return mid;
            if (nums[l] <= nums[mid]) { // isLeftPortionSorted?
                if (target >= nums[l] && target < nums[mid]) r = mid - 1; // target is in left portion
                else l = mid + 1;
            } else {
                if (target > nums[mid] && target <= nums[r]) l = mid + 1; // target is in right portion && isRightPortionSorted?
                else r = mid - 1;
            }
        }
        return -1;
